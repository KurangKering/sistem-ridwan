...
from .forms import DataLatihPemainDepanForm
...
def data_latih_index(request):
    return render(request, 'pemain_depan/data-latih/index.html')



@csrf_exempt
def data_latih_detail(request):
    id_data_latih_depan = request.POST.get('id')
    if (id_data_latih_depan == None or id_data_latih_depan == ''):
        context = context_response(False, 'id tidak boleh kosong')


    data_latih_depan = DataLatihPemain.objects.get(pk=id_data_latih_depan)
    serial = model_to_dict(data_latih_depan)
...
def data_latih_insert(request):
    ...
    form = DataLatihPemainDepanForm(request.POST)
    if form.is_valid():
       post_data = form.cleaned_data
       data_latih_depan = DataLatihPemain()
       data_latih_depan.posisi = 4
       data_latih_depan.nama = post_data['nama']
       data_latih_depan.usia = post_data['usia']
       data_latih_depan.pemain_inti = post_data['pemain_inti']
       data_latih_depan.cadangan_main = post_data['cadangan_main']
       data_latih_depan.mop = post_data['mop']
       data_latih_depan.kk = post_data['kk']
       data_latih_depan.km = post_data['km']
       data_latih_depan.gol = post_data['gol']
       data_latih_depan.assist = post_data['assist']
       data_latih_depan.pelanggaran = post_data['pelanggaran']
       data_latih_depan.dilanggar_lawan = post_data['dilanggar_lawan']
       data_latih_depan.akurasi_tembakan = post_data['akurasi_tembakan']
       data_latih_depan.akurasi_operan = post_data['akurasi_operan']
       data_latih_depan.akurasi_umpan_silang = post_data['akurasi_umpan_silang']
       data_latih_depan.sukses_dribel = post_data['sukses_dribel']

       data_latih_depan.set_bobot_data()
       data_latih_depan.save()
...
def data_latih_update(request):
    ...
    if form.is_valid():
       post_data = form.cleaned_data
       data_latih_depan = DataLatihPemain.objects.get(pk=request.POST.get('id'))
       data_latih_depan.nama = post_data['nama']
       data_latih_depan.usia = post_data['usia']
       data_latih_depan.pemain_inti = post_data['pemain_inti']
       data_latih_depan.cadangan_main = post_data['cadangan_main']
...
def pengujian_index(request):
    return render(request, 'pemain_depan/pengujian/index.html')
...
def pengujian_proses(request):
    ...
    data_latih_all_fields = DataLatihPemain.get_posisi_depan()
